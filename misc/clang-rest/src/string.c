...
String *string_create(const char *s)
{
    ...
    self->str = (char*) strdup(s);
    return self;
}

/*==========================================================================
string_create_with_len
*==========================================================================*/
...
void string_prepend(String *self, const char *s)
{
    ...
    if (self->str == NULL)
        self->str = (char*) strdup("");
    int newlen = strlen(self->str) + strlen(s) + 2;
    char *temp = (char*) strdup(self->str);
    free(self->str);
...
/*==========================================================================
string_substitute_all_in_place
*==========================================================================*/
void string_substitute_all_in_place(String *self,
                                    const char *search, const char *replace)
{
    String *n = string_substitute_all(self, search, replace);
    char *ns = (char*) strdup(n->str);
    char *temp = self->str;
    self->str = ns;
    free(temp);
    string_destroy(n);
...
List *string_tokenize(const String *s)
{
    ...
    for (i = 0; i < l; i++)
    {
        ...
        default:
            log_error("Internal error: bad state %d and char type %d in tokenize()",
                      state, chartype);
        }
    }
